...
public static class GetBranchHandlerTestData
{
    ...
    /// - CreatedBy (using internet usernames)
    /// </summary>
    private static readonly Faker<GetBranchResult> BranchResultFaker = new Faker<GetBranchResult>()
        .RuleFor(b => b.Id, f => f.Random.Guid())
        .RuleFor(b => b.Name, f => f.Company.CompanyName())
        .RuleFor(b => b.Address, f => f.Address.FullAddress())
        .RuleFor(b => b.CreatedAt, f => f.Date.Past())
    ...
    /// </summary>
    /// <returns>A valid Branch entity with randomly generated data.</returns>
    public static GetBranchResult GenerateValidResult()
    {
        return BranchResultFaker.Generate();
    }
}